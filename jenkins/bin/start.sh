...
REPO=$LOGNAME
IMAGE_NAME="forjj-sample-jenkins"
REPO=forjj-sample
...
CONTAINER_IMG="$(sudo docker ps -a -f name=forjj-sample-jenkins-dood --format "{{ .Image }}")"
...
JENKINS_MOUNT="-v forjj-sample-jenkins-home:/var/jenkins_home -e DOCKER_JENKINS_MOUNT=forjj-sample-jenkins-home:/var/jenkins_home"
...
then
    ...
    then
        ...
        echo "#!/bin/sh
sleep 30
docker rm -f forjj-sample-jenkins-dood
sleep 2

docker run --restart always $DOCKER_DOOD -d -p $SERVICE_PORT:8080 $JENKINS_MOUNT --name forjj-sample-jenkins-dood $GITHUB_USER $ADMIN $CREDS $PROXY $DOCKER_OPTS $TAG_NAME
...
sudo docker run --restart always $DOCKER_DOOD -d -p $SERVICE_PORT:8080 $JENKINS_MOUNT --name forjj-sample-jenkins-dood $GITHUB_USER $ADMIN $CREDS $PROXY $DOCKER_OPTS $TAG_NAME

if [ $? -ne 0 ]
then
    echo "Issue about jenkins startup."
    sudo docker logs forjj-sample-jenkins-dood
    exit 1
fi
